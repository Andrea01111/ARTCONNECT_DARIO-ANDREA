...
public class JdbcArtistDao implements ArtistDao {
    ...
    @Override
    public List<Artist> searchFullText(String keyword) {
        List<Artist> artists = new ArrayList<>();
        String sql = "SELECT *, MATCH(name, bio, location) AGAINST(? IN BOOLEAN MODE) AS score " +
                     "FROM Artist " +
                     "WHERE MATCH(name, bio, location) AGAINST(? IN BOOLEAN MODE) " +
                     "ORDER BY score DESC";
        try (Connection conn = ConnectionManager.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) artists.add(mapResultSetToArtist(rs));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
